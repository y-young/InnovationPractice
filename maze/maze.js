...
var start = [{x:-1, y:-1}, {x:-1, y:-1}], end = [{x:-1, y:-1}, {x:-1, y:-1}],grid = 5;
...
function drawBlock(ctx, sx, sy, a) {
    switch( a ) {
        ...
        case 4: ctx.fillStyle = "#969696"; break;
...
function solveMaze2(index) {
    if(start[index] && end[index]) {
        ...
        alert("Search failed!");
        start = [{x:-1, y:-1}, {x:-1, y:-1}], end = [{x:-1, y:-1}, {x:-1, y:-1}]
        return;
    }
...
function createMaze1() {
    ...
    if( neighbours.length < 1 ) {
        if( stacks[0].length < 1 ) {
            ...
            document.getElementById("btnCreateMaze").classList.remove("loading");
...
function onCreate() {
    ...
    document.getElementById("btnCreateMaze").classList.add("loading");
